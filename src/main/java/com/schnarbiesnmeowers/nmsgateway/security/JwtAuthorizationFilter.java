package com.schnarbiesnmeowers.nmsgateway.security;
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
		...
		if(request.getMethod().equalsIgnoreCase(Constants.OPTIONS_HTTP_METHOD)) {
			...
			if (this.jwtTokenProvider.isThisAValidToken(username,
				...
				List<GrantedAuthority> authorizations = this.jwtTokenProvider
						.getAuthorizations(token);
				// get the authentications
				Authentication userAuth = this.jwtTokenProvider
						.getUserAuthentication(username, authorizations, request);
